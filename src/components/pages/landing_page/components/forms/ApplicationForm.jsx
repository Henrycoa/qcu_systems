...
// ✅ FIXED: Use correct path to Login and Register (same folder)
...
const ApplicationForm = ({ selectedPermit, onSubmitSuccess, onBack }) => {
  ...
  const validateStep = () => {
    ...
    if (step === 4) {
      if (!formData.agreeToTerms) newErrors.agreeToTerms = "Please certify that information is correct";
    }
  ...
  const handleFinalSubmit = () => {
    if (validateStep()) {
      ...
      onSubmitSuccess(generatedRef, { ...formData, userId: currentUser?.id });
    }
  };

  const handleLoginSuccess = () => {
    const user = JSON.parse(localStorage.getItem('qc_current_user'));
    setCurrentUser(user);
    setIsLoggedIn(true);
  };
  ...
  //               {currentUser?.fullName || formData.firstName}
  //             </span>
  //             <button 
  ...
  //                     onChange={handleInputChange} 
  //                     className="w-full px-5 py-4 bg-gray-50 border-2 rounded-2xl outline-none border-transparent focus:border-[#243ead] font-bold text-gray-800" 
  ...
  //                     onChange={handleInputChange} 
  //                     className="w-full px-5 py-4 bg-gray-50 border-2 rounded-2xl outline-none border-transparent focus:border-[#243ead] font-bold text-gray-800 appearance-none"
  //                   >
  ...
  //                     onChange={handleInputChange} 
  //                     rows="4" 
  //                     className="w-full px-5 py-4 bg-gray-50 border-2 rounded-2xl outline-none border-transparent focus:border-[#243ead] font-bold text-gray-800 resize-none" 
  //                   />
  //                 </InputWrapper>
  //               </div>
  //             )}
  ...
  //                   <DocumentItem 
  //                     doc="Chest X-ray" 
  //                     subtext="Valid for 1 year" 
  //                     file={files['Chest X-ray']} 
  //                     onFileSelect={handleFileChange} 
  //                   />
  //                   <DocumentItem 
  //                     doc="Stool Exam" 
  //                     subtext="Valid for 6 months" 
  //                     file={files['Stool Exam']} 
  //                     onFileSelect={handleFileChange} 
  //                   />
  //                 </div>
  //                 {errors.files && (
  ...
  //                     <ShieldCheck className="w-5 h-5" /> Application Summary
  //                   </h3>
  ...
  //                     </p>
  //                     <p className="text-sm text-gray-600">
  //                       <span className="font-bold">Mobile:</span> {formData.mobileNumber || 'Not provided'}
  //                     </p>
  //                     <p className="text-sm text-gray-600">
  //                       <span className="font-bold">Category:</span> {formData.industry === 'food' ? 'Food Handler' : 'Non-Food Handler'}
  //                     </p>
  //                     <p className="text-sm text-gray-600">
  //                       <span className="font-bold">Documents:</span> {Object.keys(files).length} file(s) uploaded
  //                     </p>
  //                   </div>
  //                 </div>
  ...
  //                     onChange={handleInputChange} 
  //                     className="mt-1 w-6 h-6 rounded-lg text-[#243ead]" 
  //                   />
  ...
  //                 className="w-full sm:w-auto px-8 py-4 text-gray-400 font-black uppercase text-xs flex items-center justify-center gap-2 hover:bg-gray-50 rounded-2xl transition-all"
  //               >
  ...
  //                   className="w-full sm:w-auto px-10 py-4 bg-[#243ead] text-white font-black uppercase text-xs rounded-2xl shadow-xl hover:-translate-y-1 transition-all flex items-center justify-center gap-3"
  //                 >
  ...
  //                   className="w-full sm:w-auto px-10 py-4 bg-emerald-600 text-white font-black uppercase text-xs rounded-2xl shadow-xl hover:-translate-y-1 transition-all flex items-center justify-center gap-3"
  //                 >
